...
pft_lp<-function(page="Influenza",lang="de") { # the API of wikipedia takes a query and gives infrmation in json format
  ...
  check_the_links<-pft_pc(links,lang=lang,start_date = "2017-01-01",end_date = "2017-01-01",status=1)
...
update_input_wiki<-function(type_of_inputs=c("wiki_primary","wiki_linked","wiki_related"),lang=lang,df=list_of_inputs){
  ...
  if(sum(type_of_inputs=="wiki_linked")==1){
    ...
    update_wiki_linked=pft_pc(page=df[[item]]$wiki_lookup_searches$wiki_linked_terms,
                              lang=lang,
                              start_date=start.at,
                              end_date=end.at,
                              status=1)
  ...
  if(sum(type_of_inputs=="wiki_related")==1){
    ...
    update_wiki_related=pft_pc(page=df[[item]]$wiki_lookup_searches$wiki_related_terms,
                               lang=lang,
                               start_date=start.at,
                               end_date=end.at,
                               status=1)
...
reformat_wiki<-function(df,start_date=min(df[[1]]$date),end_date=max(df[[1]]$date),status=0){ # input is df= a list from pft_pc and source_list=which was "page" in pft_pc
