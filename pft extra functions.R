...
###--------------------------------------------------###
###------------pft_add_related_terms-----------------###
###--------------------------------------------------###
# name: pft_add_terms
# aim: define relevant terms by hand, add them to a term list 
# ( not for list_of_inputs!-> but for "lookup_terms" !!! )
# OUTPUT MUST BE SET TO "lookup_terms"
# 
# Arguments:
#   lang: language for wikipedia project "de", "en" etc
#   terms: one or more terms, must match wikipedia page names
#   type: a list in lookup_terms ("primary","related","linked","random_pages")
#   add: 1= adding to existing terms; 0= overwriting existing terms
# Output:
#   updating lookup_terms 

pft_add_terms<-function(type="linked",terms=c("Husten","Schnupfen"), add=1, lang="de"){
...
# example
pft_add_terms(type="linked",
              terms=c("Husten","Halsschmerzen"), 
              add=1, 
              lang="de")$de$linked

###--------------------------------------------------###
###------------pft_random_wiki_pages-----------------###
###--------------------------------------------------###
# name: pft_random_wiki_pages
# aim: Get a list of random wikipedia page names to generate the regular daily wikipedia usage
# Arguments:
#   lang: language for wikipedia project "de", "en" etc
#   number: number of random pages that should be generated
# Output:
#   a character vector with length number
#
...
# example
wiki_random_2<-pft_random_wiki_pages("de",2)

###-----------------------------------------------------------------###
###---------------pft_initiate--------------------------------------###
###-----------------------------------------------------------------###
# name: pft_random_wiki_pages
# aim: take lists from wiki_primary, wiki_linked... etc and download all relevant material into list_of_inputs
# -> Useful when new lang is being initiated
# Arguments:
#   lang: language for wikipedia project "de", "en" etc
#   start_date: From which date page views shall be downloaded
#   end_date: Until which date page views shall be downloaded
#   status: report status of downloading procedure?
#   df: where inputs shall be stored, default= list_of_inputs
# Output:
#   updated list_of_inputs
#
pft_initiate<-function(lang="nl",
  ...
  df[[item]]$wiki_linked<-wft_pc(page=df[[item]]$wiki_lookup_searches$wiki_linked_terms,
                                 lang=lang,
                                 start_date=start_date,
                                 end_date=end_date,
                                 status=status)}
  ...
  df[[item]]$wiki_related<-wft_pc(page=df[[item]]$wiki_lookup_searches$wiki_related_terms,
                                  lang=lang,
                                  start_date=start_date,
                                  end_date=end_date,
                                  status=status)}
  ...
  df[[item]]$wiki_random<-wft_pc(page=df[[item]]$wiki_lookup_searches$wiki_random_page,
                                 lang=lang,
                                 start_date=start_date,
                                 end_date=end_date,
                                 status=status)}
...
#   
# 
pft_build_up<-function(start_date=start_date, end_date=end_date,country="germany",type_of_outcome="Influenza_AB_lab_incidence_std",lang="de",type_of_input=c("wiki_primary", "wiki_related"), input_df=list_of_inputs,outcome_df=list_of_outcomes,status=1){
...
###-----------------------------------------------------------------###
# name: eval_pft_model
# aim: takes a list from pft_model_df and gives back some stats and plots(!)
# Arguments:
#   pft_model_df: where did you put your pft_model_df
#   start_date: From which date page views shall be downloaded
#   end_date: Until which date page views shall be downloaded
#   status: report status of downloading procedure?
#   df: where inputs shall be stored, default= list_of_inputs
...
eval_pft_model<-function(pft_model_df=test, # a list, created by pft_model
                         method="plot1", # "plot_mean_performance" or "plot1"
                         eval_date="20160801")
...
{ 
  ...
  if(method=="plot1"){
    ...
    eval_from_date_plot=ggplot(data=all.data,aes(x=date,y=outcome)) +
      ...
      annotate("text",x=min(all.data$date)+100, y =max(all.data.ith$outcome)-1 , 
  ...
  if(method=="plot_mean_performance"){ 
    ...
    for(d in 1:length(pft_model_df)){
      for(f in 1:length(pft_model_df[[1]]$model.stats$predicted.lm)){
        date[[f]]<-try(as.numeric(pft_model_df[[d]]$model.stats$predicted.lm[f]),silent=T)}
      nowcast_eval<-rbind(nowcast_eval,date)} # this is a confusing list with all the predictions for days 1-28
    
    for(s in 1:length(nowcast_eval[1,])){ # predcition days nee to be shifted along the time axis to be in the right column
...
# x<-eval_pft_model(pft_model_df=test, method="plot_mean_performance",eval_date="20160801")


# **__**__**__*_*_*_*_*_*_*_****____*_*_*_*____*_*_***_*_____*_*_****_#
#*#--*------*----------------*--------*---*-----------*----*-*--*---##*
##*---*---------*-  pft_model beta 1.0------*----*---*---------*----#*#
#*#----------*---------*-*-*---*-----*-------*---*----*----*----*-*-*##
#* **__**__**__*_*_*_*_*_*_*_****____*_*_*_*____*_*_***_*_____*_*_***_#
...
# name: pft_model
# aim: automate the building of predictive models. alpha version
# Arguments:
#   lang: select a lang from list_of_inputs (for build_df function) (e.g. "nl" or "de")
#   
# Output:
#   updated list_of_inputs
#
pft_model<-function(lang="nl",
                    ...
                    cv_fold="M", # "M", "Y", or 3 to sample size
                    ...
                    time_lag= -14, # how many days is official data behind? -7 for 1 week behind page view data
  ...
  if(as.character(start_date_eval)=="auto"){start.eval.at<-min(df$date)+training_period}
  ...
  if(as.character(training_period)=="past"){train.period<-1}
  ...
  if(method=="cv"){
    ...
    for(i in seq(1:max(n.eval.steps))){ # evaluation_intervall?? -1??? or 0???
      ...
      if(cv_fold=="Y"){id=as.factor(format(dates.for.validation,format="%Y"));nfolds=length(levels(id))
      cv.out=cv.glmnet(x=train.x.ith,y=train.y.ith, nfolds = nfolds, foldid = as.numeric(id))    }
      if(cv_fold=="M"){id=as.factor(format(dates.for.validation,format="%Y-%m"));nfolds=length(levels(id))
      cv.out=cv.glmnet(x=train.x.ith,y=train.y.ith, nfolds = nfolds, foldid = as.numeric(id))     }
      if(is.numeric(cv_fold) & cv_fold>=3){id=NULL; nfolds=cv_fold
